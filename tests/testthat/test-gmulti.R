...
test_that("giottoMulti setters refuse per-sample spatial content", {
    mg <- createGiottoMulti(list(a = .mk_minimal(5, 4), b = .mk_minimal(3, 4)))
    sl <- getSpatialLocations(mg@objects$b)

    # @spatial_network is the ONLY multi-level spatial slot: an edge cannot
    # be decomposed into per-sample pieces, and locations / polygons /
    # points / images each belong to exactly one sample. A multi-level copy
    # would encode that ownership as a `sample::` prefix on a string instead
    # of as where the thing lives.
    expect_error(setSpatialLocations(mg, x = sl),
        "no multi-level slot for spatial locations")
    # the refusal names the way through rather than just saying no
    expect_error(setSpatialLocations(mg, x = sl), "mg\\[\\[")
    # the old `object =` write target is gone; reaching for it hits the same
    # refusal rather than being swallowed by `...`
    expect_error(setSpatialLocations(mg, x = sl, object = "b"),
        "no multi-level slot")

    expect_error(setPolygonInfo(mg, x = sl), "no multi-level slot for polygons")
    expect_error(setFeatureInfo(mg, x = sl), "no multi-level slot for feature")
    expect_error(setGiottoImage(mg, x = sl), "no multi-level slot for images")

    # nothing was written anywhere on the way to refusing
    expect_null(mg@spatial_network)
})

test_that("setSpatialNetwork writes, and only at the multi level", {
    mg <- createGiottoMulti(list(a = .mk_minimal(5, 4), b = .mk_minimal(3, 4)))
    # a cross-sample edge has no per-sample representation, which is why
    # this one has a slot at all
    expect_error(setSpatialNetwork(mg, x = 1, object = "b"),
